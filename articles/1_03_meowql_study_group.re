...
ですので、うちの会社の書棚にあった@<href>{https://gihyo.jp/book/2016/978-4-7741-8066-3,「改訂第3版 すらすらと手が動くようになる SQL書き方ドリル」}を使うことにしました。
...
@<tt>{INSERT}や@<tt>{UPDATE}、@<tt>{CREATE TABLE}なども扱ってますが…。業務で扱えるのは、危険を伴わない@<tt>{SELECT}だけと思い、ひとつのテーブルを扱う2章、複数のテーブルを扱う3章までにしました。
...
始める前に、@<tt>{WHERE}、@<tt>{COUNT}や@<tt>{SUM}などの集計、複数テーブルをつなげる@<tt>{JOIN}あたりはつまづきそうと想定していましたが…
